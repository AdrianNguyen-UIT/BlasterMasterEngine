...
public:
	SceneManager(std::shared_ptr<DeviceResources> p_DeviceResources);
	~SceneManager();
	HRESULT CreateScenesResources();
	static bool LoadScene(std::wstring p_Name);
	static bool LoadScene(size_t index);

	static std::shared_ptr<Scene> GetActiveScene();
	static std::shared_ptr<Scene> GetSceneByName(std::wstring p_Name);
